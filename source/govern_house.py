...
class GovernHouse:
    ...
    def add_product(self, case_name, param):
        """【政府端--适老化】：添加产品"""
        print(param)
    ...
    def get_product_list(self, case_name, param):
        """【政府端--适老化】：分页查询产品列表"""
    ...
    def get_product_type_list(self, case_name):
        """【政府端--适老化】：查询产品类型list"""
        _URL = self.govern + self.house.house_govern("product", "getProductTypeList")
        response = self.request.get_request(case_name=case_name, _url=_URL, _headers=self.headers)
        return response

    def stop_start_product(self, case_name, param):
        """【政府端--适老化】：产品停启"""
        _URL = self.govern + self.house.house_govern("product", "stopStartProduct")
        response = self.request.get_request(case_name=case_name, _url=_URL, _data=param, _headers=self.headers)
        return response

    def update_product(self, case_name, param):
        """【政府端--适老化】：编辑产品信息"""
        _URL = self.govern + self.house.house_govern("product", "updateProduct")
        response = self.request.post_request_json(case_name=case_name, _url=_URL, _json=param, _headers=self.headers)
        return response

    def get_unit_data(self, case_name):
        """【政府端--适老化】：获取单位数据"""
        _URL = self.govern + self.house.house_govern("product", "getUnitData")
        response = self.request.get_request(case_name=case_name, _url=_URL, _headers=self.headers)
        return response

    def get_product_detail(self, case_name, param):
        """【政府端--适老化】：查看产品详情"""
...
# 【施工单位】
    ...
    def add_transform_apply(self, case_name, param):
        """【政府端--适老化】：添加改造申请"""
        _URL = self.govern + self.house.house_govern("apply", "addTransformApply")
        response = self.request.post_request_json(case_name=case_name, _url=_URL, _json=param, _headers=self.headers)
        return response

    def apply_transform(self, case_name, data):
        """【政府端--适老化】：改造申请"""
    ...
    def edit_transform(self, case_name, json):
        """【政府端--适老化】：编辑改造单"""
        _URL = self.govern + self.house.house_govern("apply", "editTransform")
        response = self.request.post_request_json(case_name=case_name, _url=_URL, _json=json, _headers=self.headers)
        return response

    def get_relation_data(self, case_name):
        """【政府端--适老化】：获取人员关系数据"""
        _URL = self.govern + self.house.house_govern("apply", "getRelationData")
        response = self.request.get_request(case_name=case_name, _url=_URL, _headers=self.headers)
        return response

    def get_transform_apply_list(self, case_name, data):
        """【政府端--适老化】：分页查询改造申请列表"""
        _URL = self.govern + self.house.house_govern("apply", "getTransformApplyList")
        response = self.request.get_request(case_name=case_name, _url=_URL, _data=data, _headers=self.headers)
        return response

    def get_transform_detail(self, case_name, data):
        """【政府端--适老化】：获取改造单详情"""
    ...
    def get_transform_verify_list(self, case_name, data):
        """【政府端--适老化】：分页查询改造审核列表"""
        _URL = self.govern + self.house.house_govern("verify", "getTransformVerifyList")
        response = self.request.get_request(case_name=case_name, _url=_URL, _data=data, _headers=self.headers)
        return response

    def verify_transform(self, case_name, data):
        """【政府端--适老化】：审核改造单"""
...
# 【改造施工】
    ...
    def constructor_apply(self, case_name, data):
        """【政府端--适老化】：派单施工方"""
